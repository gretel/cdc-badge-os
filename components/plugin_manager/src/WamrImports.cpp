                       ...
                       uint32_t count, uint32_t sel, uint32_t menu, bool replace)
{
    ...
    constexpr uint32_t kMaxItems = 32;
    ...
    return replace ? host_ui_replace_list(title, native, static_cast<uint16_t>(count), sel, menu)
                   : host_ui_push_list   (title, native, static_cast<uint16_t>(count), sel, menu);
}
}  // namespace
